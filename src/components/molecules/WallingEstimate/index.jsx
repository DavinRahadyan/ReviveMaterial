...
import CostHistoryWall from "@components/atoms/CostHistoryWall";
import axios from "axios";
import Cookies from "js-cookie";

const WallingEstimate = () => {
  const { addDindingEstimate } = useBudget();
  ...
  useEffect(() => {
    const fetchEstimates = async () => {
      ...
      if (token) {
        try {
          const response = await axios.get("http://localhost:8000/api/users/list_estimasi_dinding/", {
            headers: {
    ...
    if (token) {
      try {
        const response = await axios.post(
          "http://localhost:8000/api/users/save_estimasi_dinding/",
          newEstimate,
          {
            headers: {
              Authorization: `Token ${token}`,
        ...
        Swal.fire({
          icon: "success",
          title: "Berhasil",
          text: "Estimasi biaya dinding berhasil disimpan.",
          toast: true,
          position: "top-right",
          timer: 3000,
          showConfirmButton: false,
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md md:p-6">
      ...
      <div className="mt-6">
        <CostHistoryWall history={history} setHistory={setHistory} />
      </div>
    </div>
  );
};

export default WallingEstimate;
