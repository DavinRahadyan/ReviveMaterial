...
const FloorEstimateDetails = () => {
  ...
  const handleViewDetails = (estimate, index) => {
    ...
    MySwal.fire({
      ...
      html: `
        <div style="overflow-x:auto;">
          <table style="width:100%;border-collapse:collapse;text-align:left;">
            ...
            <tbody>
              ${lantaiEstimate.history.map((estimate, index) => `
                <tr>
                  <td style="border:1px solid #ddd;padding:8px;">Rp ${Number(estimate.cost).toLocaleString()}</td>
                  <td style="border:1px solid #ddd;padding:8px;">${estimate.numberOfPacks}</td>
                  <td style="border:1px solid #ddd;padding:8px;">${estimate.timestamp}</td>
      ...
      focusConfirm: false,
      didOpen: () => {
        lantaiEstimate.history.forEach((_, index) => {
          document
